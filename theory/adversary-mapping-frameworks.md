...
│ Reconnais-   │───▶│Weaponization │───▶│   Delivery   │───▶│ Exploitation │
│    sance     │    │              │    │              │    │              │
└──────────────┘    └──────────────┘    └──────────────┘    └──────────────┘
                                                                     │
┌──────────────┐    ┌──────────────┐    ┌──────────────┐            │
│   Stage 7    │    │   Stage 6    │    │   Stage 5    │            ▼
│  Actions on  │◀───│  Command &   │◀───│ Installation │◀───────────┘
│  Objectives  │    │   Control    │    │              │
└──────────────┘    └──────────────┘    └──────────────┘
      ▲
      │
      └── Defender goal: detect and disrupt before this stage is reached
```
...
```
ATT&CK Matrix (Enterprise — partial example)

┌─────────────────┬──────────────────┬──────────────────┬──────────────────┐
│  TA0001         │  TA0002          │  TA0003          │  TA0004          │
│  Reconnaissance │  Execution       │  Persistence     │  Priv. Esc.      │
├─────────────────┼──────────────────┼──────────────────┼──────────────────┤
│  T1595          │  T1059           │  T1053           │  T1055           │
│  Active Scan    │  Cmd & Scripting │  Scheduled Task  │  Process Inject. │
│  ├─ T1595.001   │  ├─ T1059.001    │  └─ T1053.005    │  ├─ T1055.001    │
│  └─ T1595.002   │  ├─ T1059.003    │                  │  └─ T1055.012    │
│                 │  └─ T1059.007    │  T1547           │                  │
│  T1598          │                  │  Boot/Logon      │  T1068           │
│  Phishing       │  T1106           │  Autostart       │  Exploit for     │
│  for Info       │  Native API      │  └─ T1547.001    │  Priv. Esc.      │
│  └─ T1598.003   │                  │                  │                  │
└─────────────────┴──────────────────┴──────────────────┴──────────────────┘
...
Hierarchy:  Tactic (TA####)
               └── Technique (T####)
                       └── Sub-technique (T####.###)
                                └── Procedure (observed real-world use)
```
...
```
        ...
        ┌─────────────────┐         ┌─────────────────┐
        │   6. DISSEMINATE │         │    2. FIX        │
        │  Share findings  │         │  Locate the data │
        │  back to intel   │         │  source that     │
        │  and det. eng.   │         │  reveals the TTP │
        └─────────────────┘         └─────────────────┘
                    ▲                           │
                    │                           ▼
        ┌─────────────────┐         ┌─────────────────┐
        │   5. ANALYZE     │         │   3. FINISH      │
        │  Review incident │         │  Build & deploy  │
        │  for new intel   │         │  the detection   │
        │  and TTPs        │         │  rule to SIEM    │
        └─────────────────┘         └─────────────────┘
                    ▲                           │
                    │    ┌─────────────────┐    │
                    └────│   4. EXPLOIT     │◀──┘
